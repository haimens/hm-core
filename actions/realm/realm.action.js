...
class VNRealmAction extends VNAction {
        ...
        try {
            ...
            await Promise.all(
                [
                    ...
                    new VNSetting().registerSetting('contact_name', 'N/A', realm_id),
                    new VNSetting().registerSetting('contact_email', `info@${realm_info.company_title}.com`, realm_id),
                    new VNSetting().registerSetting('company_address', ' ', realm_id),
    ...
    static async modifyPrimaryResourceInRealm(params, body, query) {
        try {
            ...
            const {
                message_resource_token, email_resource_token,
                payment_resource_token, address_token, tribute_rate_token
            } = body;
            const update_pack = {};
            ...
            if (tribute_rate_token) {
                const {vn_tribute_rate_id} = await new VNTributeRate(tribute_rate_token).findInstanceDetailWithToken();
                update_pack['tribute_rate_id'] = vn_tribute_rate_id;
            }

            const response = await realmObj.modifyInstanceDetailWithToken(update_pack,
                ['primary_message_resource_id', 'primary_email_resource_id',
                    'primary_payment_resource_id', 'address_id', 'tribute_rate_id']
            );
